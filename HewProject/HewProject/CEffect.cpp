...
void CEffect::Draw()
{
	...
	// 引数　①②：映し出す面の横縦の長さ　
	// 　　　③④：映し出す空間の奥行（手前と最奥の距離）
	cb.matrixProj = XMMatrixOrthographicLH(SCREEN_RATIO_W * mCamera->scaleScreen * mCamera->mTransform.scale.x, SCREEN_RATIO_H * mCamera->scaleScreen * mCamera->mTransform.scale.x, 0.0f, 3.0f);
