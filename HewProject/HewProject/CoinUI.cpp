...
#define CIRCLE_OFFSETX (1.5f)	// 〇同士のX座標差分
#define CIRCLE_OFFSETY (0.7f)	// 棒から〇のY座標差分
#define PRO_SCALETIME (1.3f)	// 大きくなるまでの時間


CoinUI::CoinUI(const int& _proteinNum, bool _isMarkActive)
{
...
void CoinUI::SetProtein(int _getProtein, bool _isDecade)
{
	if (_isDecade)
	{
...
void CoinUI::SetPosition(const Vector3& _pos)
{
...
void CoinUI::SetScale(const Vector2& _scale)
{
	allScale = _scale;
...
void CoinUI::SetActive(bool _isActive)
{
	for (int i = 0; i < stageProMax; i++)
	{
