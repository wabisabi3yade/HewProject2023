...
{
	// 移動しているときは処理しない
	if (isMoving || isRising || isFalling || isLookMap || player->GetIsStop() || isCannonMove || inCannon || isMenu || player->GetIsPlayMakeover() || player->GetGameOverOnes() || player->GetIsGameOver()) return;
